...
| DNS failure | 保留 last IP，不 render、不 restart |
| Address change | atomic render、restart、commit new state |
| Referenced file change | 相同 IP 下 certificate fingerprint 仍觸發 restart |
| Restart failure | 不提交 new IP/hash，讓下一輪可 retry |
...
改變 DDNS，watcher 應在一個 interval 內 render 並 restart Gluetun。
