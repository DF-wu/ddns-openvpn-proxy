...
以純 Docker Compose 部署的 DDNS-aware OpenVPN HTTP／SOCKS5 proxy。專案不含
Dockerfile、不建置自有 image；遠端主機只要放入 OpenVPN profile，便能直接
`docker compose up`。

Gluetun 的 custom OpenVPN provider 要求 `remote` 必須是 IPv4，無法直接接受
DDNS hostname。本專案只補上這個缺口：解析 hostname、產生 Gluetun 可讀的
runtime profile，並在 A record 或 profile／憑證改變時安全重啟 Gluetun。VPN、
kill switch、連線健康檢查、HTTP proxy 與 SOCKS5 都交給成熟的上游服務。

![DDNS OpenVPN Proxy architecture](docs/assets/architecture.svg)
...
- **自動恢復**：DDNS IP、`.ovpn` 或被引用的憑證／金鑰改變後，原子更新 runtime
...
| 項目 | 支援內容 |
| --- | --- |
| VPN | OpenVPN custom profile，IPv4 endpoint |
| DDNS | 單一 hostname，可回傳一個或多個 IPv4 A record |
| Profile | 一個啟用中的 `remote` directive |
| Proxy | HTTP CONNECT、SOCKS5 TCP CONNECT |
| 平台 | Linux Docker Engine、Compose v2、`/dev/net/tun` |
| 架構 | 上游 image 支援的 amd64、arm64 等平台 |

WireGuard、IPv6 endpoint 與 SOCKS5 UDP ASSOCIATE 不在目前契約內。這個範圍是刻意
保持小而明確，避免把 OpenVPN DDNS 問題擴張成另一套 VPN 平台。
...
1. watcher 每 `DDNS_POLL_SECONDS` 秒解析一次 DDNS，並計算 profile 與引用檔案的
   SHA-256 fingerprint。
...
| 元件 | 預設版本 | 責任 |
| --- | --- | --- |
| [Gluetun](https://github.com/qdm12/gluetun) | `v3.41.1` | OpenVPN、kill switch、health、HTTP proxy |
